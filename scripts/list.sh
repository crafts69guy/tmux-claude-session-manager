#!/usr/bin/env bash
# Open the session picker in a popup.
#
# tmux does not allow nested sessions, so the picker always runs on a client that
# is NOT attached to a managed session. When pressed from inside a session popup,
# detach that popup's client first, then host the picker on the outer client;
# picker.sh morphs the chosen session in place.
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=helpers.sh
. "$DIR/helpers.sh"

prefixes_re="^($(provider_prefixes_regex))"
read -r w h < <(popup_dims)

# The session of a client attached to a managed (provider-prefixed) session —
# i.e. the popup we are inside, if any. Empty when invoked from a normal pane.
nested_session() {
  tmux list-clients -F '#{client_name} #{session_name}' 2>/dev/null |
    awk -v re="$prefixes_re" '$2 ~ re { print $2; exit }'
}

# A client NOT attached to a managed session — the outer client that should host
# the picker popup.
host_client() {
  tmux list-clients -F '#{client_name} #{session_name}' 2>/dev/null |
    awk -v re="$prefixes_re" '$2 !~ re { print $1; exit }'
}

# If we are inside a session popup, close it (detach its client) and wait until
# the session is gone before opening the picker on the outer client.
sess="$(nested_session)"
if [ -n "$sess" ]; then
  tmux detach-client -s "$sess"
  for _ in $(seq 1 100); do
    [ -z "$(nested_session)" ] && break
    sleep 0.05
  done
fi

host="$(host_client)"
tmux set-option -g @ai_parent "$host"

# Host the picker on the outer client (it has no popup open now); fall back to
# the default client if none was found.
if [ -n "$host" ]; then
  tmux display-popup -c "$host" -w "$w" -h "$h" -E "$DIR/picker.sh"
else
  tmux display-popup -w "$w" -h "$h" -E "$DIR/picker.sh"
fi
